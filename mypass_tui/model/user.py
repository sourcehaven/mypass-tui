...
class User:
    ...
    @classmethod
    def join_auth_url(cls, endpoint: str):
        return urljoin(cls.auth_url, endpoint)

    @classmethod
    def join_vault_url(cls, endpoint: str):
        return urljoin(cls.vault_url, endpoint)
    ...
    def login(cls, username: str, password: str):
        session = requests.Session()
        resp = session.post(url=cls.join_auth_url("login"), json={USERNAME: username, PASSWORD: password})
    ...
    def registration(
        ...
        session = requests.Session()
        resp = session.post(
            url=cls.join_auth_url("registration"),
            json={
                USERNAME: username,
                PASSWORD: password,
    ...
    def logout(self):
        resp = requests.delete(url=self.join_auth_url("logout"), auth=self.auth())
    ...
    def refresh(self) -> None:
        resp = requests.post(url=self.join_auth_url("refresh"), auth=self.auth())
    ...
    def vault_add(self, entry: VaultEntry):
        resp = requests.post(url=self.join_vault_url("add"), json={"fields": entry.to_dict()}, auth=self.auth())
    ...
    def vault_read(self) -> list[VaultEntry]:
        resp = requests.post(url=self.join_vault_url("select"), json={}, auth=self.auth())
    ...
    def vault_delete(self, crit: dict[str, str]) -> dict[str, Any]:
        resp = requests.post(url=self.join_vault_url("delete"), json={"crit": crit}, auth=self.auth())
